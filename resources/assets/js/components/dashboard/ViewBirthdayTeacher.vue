<template>
    <div class="mb-2">
        ...
        <h1 class="text-sm uppercase text-gray-800 font-semibold mb-2">Today's Birthday (Teachers)</h1>
        <div>
            <div class="flex border h-24 overflow-hidden">
                <img :src="url+'/uploads/birthday.png'" class="w-1/5 md:w-1/5 lg:w-2/5 border-r">
                <ul class="list-reset  px-3 w-3/5">
                    <marquee direction="up" onmouseover="this.stop();" onmouseout="this.start();">
                        <li class="flex items-center py-2" v-for="user in users">  
                            ...
                            <a :href="url+'/admin/teacher/show/'+user['name']" class="mx-2 text-gray-800 text-sm" v-if="mode == 'admin'">{{ user['firstname'] }} 
            ...
            <div class="flex border py-1 text-center bg-gray-500" v-if="Object.keys(this.users).length > 0 && this.mode == 'admin'">
                <a :href="url+'/'+mode+'/dashboard/birthday/teacher'" class="text-white px-2 font-medium uppercase w-full text-xs py-1" title="Send Message">Send a Message</a> 
            </div>
        </div>  
    </div>
</template>
...
<script>
    export default {
        ...
        methods: 
        {
            getMember()
            {
                axios.get('/'+this.mode+'/dashboard/showBirthdayTeacher').then(response => {
